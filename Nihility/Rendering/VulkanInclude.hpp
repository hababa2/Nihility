...
{												\
    VkResult _result = VkResult(expr);			\
	if (!ResultSuccess(_result))				\
	{											\
		Logger::Error(ResultString(_result));	\
	}											\
}
...
{												\
	...
	if (!ResultSuccess(_result))				\
	{											\
		Logger::Fatal(ResultString(_result));	\
	}											\
}
...
{												\
    VkResult _result = VkResult(expr);			\
	if (!ResultSuccess(_result)) { return; }	\
}
